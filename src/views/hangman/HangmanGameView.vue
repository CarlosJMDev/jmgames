...
<template>
  <div class="flex flex-col items-center mt-20 text-white gap-8">
    <h1 class="text-4xl font-bold">{{ i18n.t('hangman.title') }}</h1>
    ...
    <div v-else class="flex flex-col justify-center items-center gap-4 font-bold">
      <h2 v-if="isWinner" class="text-green-500">
        {{ i18n.t('hangman.congratulations') }}
      </h2>
      <h2 v-else class="text-red-700">{{ i18n.t('hangman.sorry') }} {{ word }}</h2>
      <button @click="newGame" class="btn">{{ i18n.t('common.newGame') }}</button>
    </div>
  </div>
</template>
